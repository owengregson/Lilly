...
def main():
    ...
    n_steps = 3  # load model + load data + save metrics
    if args.tier >= 1:
        n_steps += 1
    if args.tier >= 2:
        n_steps += 1
    ...
    ui = ProgressUI(n_steps, animator)
    animator.start()
    ...
    if args.tier >= 1:
        ...
        metrics["tier1"] = t1
        acc = t1.get("action_accuracy", 0)
        ui.done(label, f"action_accuracy={acc:.4f}")

    # Step: Tier 2
    if args.tier >= 2:
        label = "Tier 2: Distributional metrics"
        ui.begin(label)
        ...
        metrics["tier2"] = t2
        ui.done(label, f"{len(t2)} metrics computed")

    # Step: Tier 3
    if args.tier >= 3:
        label = "Tier 3: Realism metrics"
        ui.begin(label)
        ...
        t3.update(style)
        metrics["tier3"] = t3
        ui.done(label, f"{len(t3)} metrics computed")
    ...
    if args.visualize:
        ...
        for batch_inputs, batch_labels in test_ds:
            ...
            for b in range(len(mask)):
                for ts in range(mask.shape[1]):
                    if mask[b, ts] > 0:
                        all_true_actions.append(true_actions[b, ts])
                        all_pred_actions.append(pred_actions[b, ts])
    ...
    # Step: Save metrics
    label = "Saving metrics"
    ui.begin(label)
    metrics_path = output_dir / "eval_metrics.json"
    with open(metrics_path, "w") as f:
        json.dump(metrics, f, indent=2)
    ui.done(label, str(metrics_path.name))

    animator.stop()
    ui.finish()

    print()
    print(f"  {t.GREEN}Evaluation complete.{t.RESET}")
